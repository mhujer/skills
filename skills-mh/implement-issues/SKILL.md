...
2. Create a branch with a short name representing the feature
3. Queue the `ready-for-agent` issues. Anything with another status stays out, and so does anything blocked by an issue you left out: its blocker will never resolve in this run.
4. Work the queue in dependency order, blockers first, one issue at a time. An issue is ready to dispatch once every issue it is blocked by is `resolved`. Dispatch it as a subagent (the Agent tool) whose prompt tells it to call the Skill tool with `implement`, passing the issue path but with extra instruction to not do code review using /code-review (will be done at the end)
5. When each subagent finishes, check that it committed something to git. A subagent that lands no commit has not finished. Halt there, and report what ran, what did not, and what you left out of the queue.
6. Unless the subagent did it itself, mark the issue as `resolved` and amend the subagent's commit.
...
Offer to merge the branch into main (with merge commit).
