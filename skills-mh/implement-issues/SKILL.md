...
3. Work the queue in dependency order, blockers first, one issue at a time. An issue is ready to dispatch once every issue it is blocked by is `resolved`. Dispatch it as a subagent (the Agent tool) whose prompt tells it to call the Skill tool with `implement`, passing the issue path.
